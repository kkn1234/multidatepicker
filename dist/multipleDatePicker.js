...
(function (angular) {
    ...
    var multipleDatePicker = function () {
            return {
                ...
                '<div class="text-center picker-navigate picker-navigate-left-arrow" ng-class="{\'disabled\':disableBackButton}" ng-click="changeMonth($event, disableBackButton, -1)"><i class="fa fa-angle-left" aria-hidden="true"></i></div>' +
                ...
                '</div>' +
                '<div class="text-center picker-navigate picker-navigate-right-arrow" ng-class="{\'disabled\':disableNextButton}" ng-click="changeMonth($event, disableNextButton, 1)"><i class="fa fa-angle-right" aria-hidden="true"></i></div>' +
                '</div>' +
                '<div class="picker-days-week-row">' +
                '<div class="text-center" ng-repeat="day in daysOfWeek">{{day}}</div>' +
                '</div>' +
                '<div class="picker-days-row">' +
                '<div class="text-center picker-day {{getDayClasses(day)}}" title="{{day.title}}" ng-repeat="day in days" ng-click="toggleDay($event, day)" ng-mouseover="hoverDay($event, day)" ng-mouseleave="dayHover($event, day)" mdp-right-click="rightClicked($event,day)"><span>{{day ? day.mdp.otherMonth && !showDaysOfSurroundingMonths ? \'&nbsp;\' : day.date.format(\'D\') : \'\'}}</span><div class="slot">{{day.price}} </div> <div class="slot">{{day.slot}}</div></div>' +
                '</div>' +
                '</div>',
                link: function (scope) {
                    ...
                    var checkNavigationButtons = function () {
                        ...
                        getDaysOfWeek = function () {
                            ...
                            var momentDaysOfWeek = moment().localeData()._weekdaysShort,
                                days = [];
                            for (var i = 1; i < 7; i++) {
                    ...
                    scope.generate = function () {
                        ...
                        var firstDayOfMonth = moment(scope.month).date(1),
                            ...
                            createDate = function () {
                                ...
                                if (angular.isDefined('slotData')) {             
                                    if(scope.slotData)    {
                                        if(scope.slotData.length){
                                            angular.forEach(scope.slotData, function(value){
                                                var calenderdate = value.calenderdate;
                                                var check = moment(calenderdate);
                                                var monthCal = check.format('M');
                                                var dayCal   = check.format('D');
                                                var yearCal  = check.format('YYYY');

                                                var viewMonthCmp = day.date.format('M');
                                                var viewDayCmp = day.date.format('D');
                                                var viewYearCmp = day.date.format('YYYY');

                                                if(viewYearCmp === yearCal){
                                                    if(viewMonthCmp === monthCal){
                                                        if(viewDayCmp === dayCal){
                                                            day.price = '$ ' + value.price;
                                                            if(value.slotsAvailable){
                                                            	day.slot = value.slotsAvailable + ' left';
                                                            }else{
                                                            	day.slot ='sold out';
                                                            	}
                                                            }
